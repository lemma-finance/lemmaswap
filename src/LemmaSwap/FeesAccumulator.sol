...
contract FeesAccumulator is AccessControl {
    ...
    function distibuteFees(address _token, uint24 _swapFee, uint256 _swapMinAmount)
        external
        onlyRole(FEES_TRANSFER_ROLE)
    {
        ...
        usdl.depositToWExactCollateral(address(xusdl), collateralAmount, dexIndex, 0, IERC20(_token));
        ...
        ILemmaSynth(_sa.synthAddress).depositToWExactCollateral(
            address(_sa.xSynthAddress), collateralAmount, 0, 0, IERC20(usdl.perpSettlementToken())
        );
    }
    ...
    function _convertCollateralToValidDexIndex(address collateral, bool isUsdl) internal view returns (uint256) {
        require(collateral != address(0), "!collateral");
        if (isUsdl) {
            require(collateralToDexIndexForUsdl[collateral] != 0, "Collateral not supported for usdl");
            return collateralToDexIndexForUsdl[collateral] - 1;
        } else {
            require(collateralToDexIndexForSynth[collateral] != 0, "Collateral not supported for synth");
